...
def safe_reply(bot, message, text):
    try:
        ...
        bot.reply_to(message, formatting.escape_markdown(text_with_compulsory), parse_mode='MarkdownV2')
        ...
        if "message to be replied not found" in str(e):
            ...
            text_with_compulsory = append_compulsory_message(text)
            bot.send_message(message.chat.id, formatting.escape_markdown(text_with_compulsory), parse_mode='MarkdownV2')
        else:
...
- /myinfo - show full info of yourself 
...
def main():
    try:
        ...
        log_error(f"Main loop error: {str(e)}", "SYSTEM", "SYSTEM")
        save_data()
        raise
